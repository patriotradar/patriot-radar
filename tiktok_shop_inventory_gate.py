"""
TikTok Shop inventory gate — human-in-the-loop product availability checks.

Detects when a matched product is missing from the creator's TikTok Shop catalog,
pauses ONLY the product attachment step, and allows resume after manual Showcase onboarding.
...
def build_inventory_gap_notification(availability: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "category": availability.get("category") or "general",
        "message": f"Product '{product_name}' is not in your TikTok Shop catalog. Add it to Showcase to enable attachment.",
        "status": "waiting_user_action",
        "action_required": "add_to_showcase",
    }
