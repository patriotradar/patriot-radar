...
Validates contract guarantees, fail-safety, and the snapshot → assembler flow.
"""
...
class TestLiveStateContract(unittest.TestCase):
    ...
    def test_primary_action_shape(self):
        state = _empty_contract()
        for key in ("label", "action", "context_id"):
...
class TestSnapshotDrivenFlow(unittest.TestCase):
    ...
    def test_snapshot_then_assemble_deterministic(self):
        ...
        state = assembleLiveState(snapshot)
        self.assertEqual(state["today_flow"]["status"], "ready_for_content")
        self.assertEqual(state["primary_action"]["action"], "generate_content")
    ...
    def test_inventory_gap_blocks_flow(self):
        ...
        self.assertEqual(state["primary_action"]["action"], "resolve_inventory_gap")
