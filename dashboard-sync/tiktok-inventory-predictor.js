/**
 * TikTok Shop Predictive Inventory Intelligence — client-side mirror.
 *
 * Predicts likely-needed products BEFORE content generation, pre-checks catalog,
 * and emits inventory_prevention_event dashboard suggestions.
 *
 * Works alongside tiktok-shop-inventory-gate.js (reactive fallback at attachment).
 */
(function () {
  "use strict";
  ...
  function precheckCatalog(likelyNeeded, catalog) {
    return (likelyNeeded || []).map(function (product) {
      ...
      return {
        ...
        catalog_status: availability.attachable ? "ready_to_attach" : "pre_add_required",
  ...
  function render(intelligence) {
    ...
    if (!intelligence || !intelligence.inventory_prevention_events.length) {
      if (!intelligence || !intelligence.must_add_products.length) {
        el.innerHTML = "";
        return;
      }
    }

    var mustAdd = intelligence.must_add_products || [];
    var prevention = intelligence.inventory_prevention_events || [];
    ...
    var h = '<div class="card" style="border-color:rgba(251,191,36,.35);margin-bottom:12px">';
    h += '<div class="card-header"><h2>Predictive Inventory Intelligence</h2>';
    h += '<div class="section-icon" style="color:#fbbf24">&#9889;</div></div>';
    h += '<p style="font-size:12px;color:var(--muted);margin-bottom:10px">';
    h += 'Products predicted BEFORE content generation. Add high-demand items to Showcase now to maximise attach rate.';
    h += '</p>';
    ...
    } else if (prevention.length) {
      for (var j = 0; j < Math.min(prevention.length, 3); j++) {
        var p = prevention[j];
        h += '<div style="padding:10px;background:rgba(251,191,36,.06);border:1px solid rgba(251,191,36,.2);border-radius:10px;margin-bottom:8px">';
        h += '<div style="font-size:14px;font-weight:900">' + escapeHtml(p.product_name) + '</div>';
        h += '<div style="font-size:11px;color:var(--white);margin-top:6px">' + escapeHtml(p.message) + '</div>';
        h += '</div>';
      }
    }

    h += '<p style="font-size:9px;color:var(--muted);margin-top:8px;text-align:center">';
    h += intelligence.ready_count + ' product(s) ready to attach | ' + intelligence.pre_add_required_count + ' need Showcase onboarding';
    h += '</p></div>';
  ...
  function processTrendResults(results, emerging, productTrends) {
    ...
    if (window.TikTokShopInventoryGate && typeof window.TikTokShopInventoryGate.processTrendResults === "function") {
      window.TikTokShopInventoryGate.processTrendResults(results);
    }
