...
(function () {
  ...
  function commerceEnabled() {
    return window.CommerceMode && typeof window.CommerceMode.isCommerceMode === "function" && window.CommerceMode.isCommerceMode();
  }

  function render(catalogGaps) {
    if (!commerceEnabled()) {
      var elOff = document.getElementById(MOUNT_ID);
      if (elOff) elOff.innerHTML = "";
      return;
    }
  ...
  function processTrendResults(results) {
    if (!commerceEnabled()) {
      render([]);
      return { gaps: [], attachments: [], inventory_gaps: [] };
    }
    ...
    render(gaps);
    return { gaps: gaps, attachments: attachments, inventory_gaps: gaps };
  }
