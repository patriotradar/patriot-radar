...
def _derive_today_flow(
    ...
    *,
    ...
    if inventory_gaps and action in ("prevent_inventory_stockout", "resolve_inventory_gap"):
        today_flow["status"] = "blocked"
        return today_flow

    if approvals and action in ("approve_queued_content", "approve_content"):
        today_flow["status"] = "awaiting_approval"
        return today_flow

    if action in ("generate_content_from_products", "create_product_content", "create_trend_content"):
        today_flow["status"] = "ready_for_content"
        return today_flow

    if action in ("match_products_to_trends", "monetise_trending_topic"):
...
def assembleLiveState(
    ...
    *,
    ...
    resolved_commerce_mode = commerce_mode
    if resolved_commerce_mode is None:
        resolved_commerce_mode = user_role.strip().lower() in ("business", "commerce", "shop")
    ...
    orchestration_input = {
        ...
        "commerce_mode": resolved_commerce_mode,
        "user_role": user_role,
    ...
    state["commerce_mode"] = resolved_commerce_mode
    state["user_role"] = _as_string(user_role, "creator")
